...
const App: React.FC = () => {
  // Navigation State
  const [activeTab, setActiveTab] = useState<'HOME' | 'PROFILE' | 'STATS'>(
    'HOME'
  );
  ...
  useEffect(() => {
    if (userProfile)
      localStorage.setItem('nissan_profile_v2', JSON.stringify(userProfile));
  }, [userProfile]);
  ...
  const handleProcess = useCallback(
    async (textOverride?: string) => {
      ...
      try {
        ...
        // Handle Food Entry
        if (
          result.action_type === 'FOOD_ENTRY' &&
          result.food_items &&
          result.food_items.length > 0
        ) {
          // Map all detected items to HistoryItems
          const newQueueItems: HistoryItem[] = result.food_items.map(
            (item, index) => ({
        ...
        // Handle Conversational Profile Setup
          ...
          result.action_type === 'PROFILE_SETUP' &&
          result.user_stats &&
          result.calculated_goal
        ) {
  ...
  const calculateAndSaveProfile = () => {
    ...
    setActiveTab('HOME');
    setLastAdvice(
      'پروفایل شما با موفقیت ذخیره شد. حالا می‌تونید وعده‌های غذایی رو ثبت کنید.'
    );
  };
  ...
  const handleSmartRecalculate = async () => {
    ...
    try {
      ...
      if (result.food_items && result.food_items.length > 0) {
        const totalCal = result.food_items.reduce(
          (s, i) => s + (i.calories || 0),
          0
        );
        const totalPro = result.food_items.reduce(
          (s, i) => s + (i.protein || 0),
          0
        );
        const totalCarb = result.food_items.reduce(
          (s, i) => s + (i.carbs || 0),
          0
        );
        const totalFat = result.food_items.reduce(
          (s, i) => s + (i.fat || 0),
          0
        );
  ...
  const cancelOrSkipItem = () => {
    ...
    if (
      ...
      editingItem &&
      reviewQueue[0].id === editingItem.id
    ) {
      const nextQueue = reviewQueue.slice(1);
      setReviewQueue(nextQueue);
      if (nextQueue.length > 0) {
  ...
  const isNewItem =
    editingItem && !history.some((h) => h.id === editingItem.id);
  const isQueueActive =
    reviewQueue.length > 0 &&
    editingItem &&
    reviewQueue[0].id === editingItem.id;
  ...
  return (
    <div
      className="min-h-screen bg-slate-50 font-['Vazirmatn'] pb-24"
      dir="rtl"
    >
      {/* --- HEADER --- */}
      <header
        className={`transition-all duration-500 p-4 shadow-lg sticky top-0 z-40 ${
          userProfile ? 'bg-emerald-600' : 'bg-slate-800'
        } text-white`}
      >
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-white/90 font-bold -mt-0.5 opacity-90">
                کالری شمار هوشمند
              </span>
            </div>
          </div>
          {userProfile && activeTab === 'HOME' && (
            <div className="text-center">
              <div className="text-[10px] text-emerald-200 uppercase font-bold">
                باقیمانده
              </div>
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateProfileField('age', Number(e.target.value))
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateProfileField('height', Number(e.target.value))
                      }
                      className="w-full bg-slate-50 border-2 border-slate-100 focus:border-emerald-400 rounded-xl p-3 font-bold text-slate-700 text-center outline-none"
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateProfileField('weight', Number(e.target.value))
                    }
                  ...
                  <div className="space-y-2">
                    {ACTIVITY_LEVELS.map((level) => (
                      <div
                        key={level.value}
                        onClick={() =>
                          updateProfileField('activity_level', level.value)
                        }
                        className={`p-3 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-3 ${
                          profileForm.activity_level === level.value
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-black text-slate-800 w-12 text-center">
                      {profileForm.weight_loss_per_month}
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-2 text-justify leading-4">
                    توصیه: کاهش وزن ایمن بین ۰.۵ تا ۴ کیلوگرم در ماه است. اعداد
                    بالاتر ممکن است برای سلامتی مضر باشد.
                  </p>
                </div>
                ...
                <button
                  onClick={() => {
                    if (
                      window.confirm(
                        'آیا مطمئن هستید؟ تمام اطلاعات شما (پروفایل و تاریخچه) پاک خواهد شد.'
                      )
                    ) {
                      localStorage.removeItem('nissan_profile_v2');
                      localStorage.removeItem('nissan_history_v2');
                      setUserProfile(null);
                      setHistory([]);
                      setProfileForm({
                        age: 30,
                  ...
                  className="w-full bg-red-50 text-red-600 py-3 rounded-xl font-bold border-2 border-red-100 hover:bg-red-100 active:scale-95 transition-all"
                >
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {lastAdvice && (
              <div className="bg-white p-4 rounded-2xl border-r-4 border-emerald-500 shadow-sm flex gap-3 items-start">
                ...
                <p className="text-sm font-bold text-emerald-800 leading-6">
                  {lastAdvice}
                </p>
              </div>
            )}
            ...
            <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm">
              ...
              <div className="w-full h-4 bg-slate-100 rounded-full overflow-hidden mb-3">
                <div
                  className={`h-full transition-all duration-1000 ${
                    consumedToday > userProfile.tdee
                      ? 'bg-red-500'
                      : 'bg-emerald-500'
                  }`}
                  style={{
                    width: `${Math.min(
                      100,
                      (consumedToday / userProfile.tdee) * 100
                    )}%`,
                  }}
                ></div>
              </div>
              ...
              <div className="flex justify-between gap-2 pt-2 border-t border-slate-50">
                <div className="flex flex-col items-center flex-1">
                  ...
                  <span className="text-sm font-black text-slate-700 bg-orange-50 px-2 py-1 rounded-lg w-full text-center border border-orange-100">
                    {consumedMacros.protein}g
                  </span>
                </div>
                <div className="flex flex-col items-center flex-1">
                  <span className="text-[10px] text-slate-400 font-bold mb-1">
                  ...
                  <span className="text-sm font-black text-slate-700 bg-blue-50 px-2 py-1 rounded-lg w-full text-center border border-blue-100">
                ...
                <div className="flex flex-col items-center flex-1">
                  ...
                  <span className="text-sm font-black text-slate-700 bg-yellow-50 px-2 py-1 rounded-lg w-full text-center border border-yellow-100">
                    {consumedMacros.fat}g
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 pb-4">
              <h3 className="font-bold text-slate-400 text-sm px-2">
                وعده‌های امروز
              </h3>
              {history.filter((h) => h.action_type === 'FOOD_ENTRY').length ===
              0 ? (
                ...
                history
                  .filter((h) => h.action_type === 'FOOD_ENTRY')
                  .slice()
                  .reverse()
                  .map((item) => (
                    <div
                      key={item.id}
                      className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3 transition-all hover:shadow-md"
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded-md font-black tracking-wide ${getMealColorClass(
                                  item.meal_type
                                )}`}
                              >
                        ...
                        <div className="flex flex-col items-end gap-3 pl-1">
                          <div className="text-right">
                            <span className="font-black text-emerald-600 text-lg block leading-none">
                              {item.calories}
                            </span>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-1">
                        <div className="bg-orange-50 rounded-lg p-1.5 text-center">
                          <div className="text-[9px] text-orange-400 font-bold mb-0.5">
                            پروتئین
                          </div>
                        ...
                        <div className="bg-blue-50 rounded-lg p-1.5 text-center">
                          <div className="text-[9px] text-blue-400 font-bold mb-0.5">
                            کربوهیدرات
                          </div>
                        ...
                        <div className="bg-yellow-50 rounded-lg p-1.5 text-center">
                          <div className="text-[9px] text-yellow-500 font-bold mb-0.5">
                            چربی
                          </div>
          ...
          <div className="animate-in fade-in duration-500 space-y-6">
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
              ...
              <p className="text-xs text-slate-400 mb-6">
                عملکرد هفته جاری (شنبه تا جمعه)
              </p>

              {/* Chart 1: Calories */}
              <div className="mb-8">
                <h3 className="text-sm font-bold text-emerald-700 mb-4 bg-emerald-50 inline-block px-3 py-1 rounded-lg">
                  روند مصرف کالری
                </h3>
              ...
              {/* Chart 2: Macros */}
              <div>
                <h3 className="text-sm font-bold text-slate-700 mb-4 bg-slate-50 inline-block px-3 py-1 rounded-lg">
                  ترکیب درشت مغذی‌ها
                </h3>
                <div className="h-64 w-full -mr-4">
                  <ResponsiveContainer width="100%" height="100%">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 pb-safe pt-2 px-6 flex justify-around items-center z-50 h-20 shadow-[0_-5px_10px_rgba(0,0,0,0.02)]">
        ...
        >
          <div
            className={`p-3 rounded-2xl ${
              activeTab === 'STATS' ? 'bg-blue-100 shadow-lg' : 'bg-transparent'
            }`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            {isQueueActive && (
              <div className="mb-4 bg-slate-100 rounded-full h-2 w-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${
                      ((originalQueueLength - reviewQueue.length + 1) /
                        originalQueueLength) *
                      100
                    }%`,
                  }}
            ...
            {isNewItem && (
              <div className="bg-amber-50 text-amber-700 p-3 rounded-xl text-xs font-bold mb-4 text-center border border-amber-100">
                🤖 من مقدار را {editingItem.amount} گرم فرض کردم.
                <br />
                لطفاً مقدار دقیق مصرفی خود را در زیر وارد کنید.
              </div>
            )}

            <div className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-400 font-bold mr-1">
                  نام غذا
                </label>
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-50 border rounded-xl p-3 font-bold text-sm text-black"
                />
              </div>
                ...
                <div className="flex gap-1 bg-slate-100 p-1 rounded-xl overflow-x-auto">
                  {MEAL_TYPES.map((meal) => (
                    <button
                      ...
                      className={`flex-1 py-2 px-1 rounded-lg text-[10px] font-bold transition-all whitespace-nowrap ${
                        editFormMealType === meal.type
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="text-[10px] text-emerald-600 font-bold mr-1">
                    کالری (با توجه به مقدار بالا)
                  </label>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-emerald-200 focus:border-emerald-500 rounded-xl p-3 font-bold text-sm text-center text-black"
                    />
                    <button
                      onClick={handleSmartRecalculate}
                      disabled={isCalculatingEdit}
                      className="bg-indigo-100 text-indigo-600 rounded-xl px-4 text-xl shrink-0"
                    >
                      {isCalculatingEdit ? '⌛' : '🤖'}
                    </button>
                  </div>
                ...
                <div>
                  <label className="text-[10px] text-orange-500 font-bold mr-1">
                    پروتئین
                  </label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-slate-50 border focus:border-orange-300 rounded-xl p-2 font-bold text-xs text-center text-black"
                  />
                </div>
                <div>
                  <label className="text-[10px] text-blue-500 font-bold mr-1">
                    کربوهیدرات
                  </label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-slate-50 border focus:border-blue-300 rounded-xl p-2 font-bold text-xs text-center text-black"
                  />
                </div>
                <div className="col-span-2">
                  <label className="text-[10px] text-yellow-600 font-bold mr-1">
                    چربی
                  </label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-slate-50 border focus:border-yellow-300 rounded-xl p-2 font-bold text-xs text-center text-black"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-100 mt-2">
                <button
                  onClick={cancelOrSkipItem}
                  className="flex-1 py-3 text-slate-400 font-bold bg-slate-50 rounded-xl text-sm"
                >
                  {isQueueActive ? 'نادیده گرفتن 🗑️' : 'لغو'}
                </button>
                <button
                  onClick={saveEditItem}
                  className="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-lg text-sm"
                >
                  {isQueueActive
